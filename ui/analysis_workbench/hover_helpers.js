...
function getRegionLabelMapUrl(sample) {
...
function summarizeUnavailableHover(sample) {
...
function getHoverRasterSource(sample, sourceId = "review") {
  const pixelMap = sample?.pigment_prediction?.pixel_map;
  if (pixelMap) return { kind: "pigment-pixel-map", url: pixelMap };
...
function summarizePixelPigment(sample, classId) {
  const names = sample?.pigment_prediction?.class_names ?? [];
  const label = names[Number(classId) - 1] ?? null;
  if (!label) return { active: false };
  return {
    active: true,
    label,
    text: `paint + ${label}`,
    detailText: `${sample?.pigment_prediction?.version_id ?? "background4"} 像素级预测`,
    scoreText: "",
    marginText: "",
    candidateText: "",
  };
...
function summarizeHoverRegion(sample, clusterId) {
  const summaries = getRegionSummaries(sample);
...
function resolveContainViewport(naturalWidth, naturalHeight, clientWidth, clientHeight) {
...
function mapPointerToImagePixel({ clientX, clientY, rect, naturalWidth, naturalHeight }) {
...
window.HoverHelpers = { getHoverRasterSource, getRegionLabelMapUrl, summarizeHoverRegion, summarizePixelPigment, summarizeUnavailableHover, resolveContainViewport, mapPointerToImagePixel };
